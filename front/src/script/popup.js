export default class Popup {
    ...
    constructor(propertiesMesto, propertiesProfil, cardList, profil, formValidator, api) {
    ...
    open(event) {
        ...
        if (event.target.classList.contains('place-card__image')) {
            ...
            document.querySelector('.popup__content').setAttribute('style', imagePath);
        } else {
            ...
            //Обработчик отправление данных формы
                ...
                document.querySelector('.popup')
                    .addEventListener('input', this.formValidator.setEventListeners.bind(this.formValidator));

                //Устанавливаем статус кнопки формы при открытии popup
                this.formValidator.setSubmitButtonState(document.querySelector('.popup__form'));
    ...
    sendForm(event) {
        ...
        if (event.target.getAttribute('name') === "new") {
            ...
            this.api.sendNewCard(popupForm.elements)
                .then(() => {
